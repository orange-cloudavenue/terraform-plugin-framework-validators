...
func TestRequireIfAttributeIsOneOfValidator(t *testing.T) {
	...
	testCases := map[string]testCase{
		"baseString": {
			...
			expError:        true,
			expErrorMessage: "If foo attribute is set and the value is \"excepted value\" this attribute is REQUIRED",
		},
		"extendedString": {
			req: internal.RequireIfAttributeIsOneOfRequest{
				ConfigValue:    types.StringNull(),
		...
		"baseBool": {
			...
			expError:        true,
			expErrorMessage: "If foo attribute is set and the value is true this attribute is REQUIRED",
		},
